...
const Features = () => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsVisible(true);
          observer.unobserve(entry.target); // trigger hanya sekali
        }
      },
      { threshold: 0 } // lebih sensitif agar muncul di HP
    );
  ...
  return (
    <section 
      ref={sectionRef}
      id="features"
      className="min-h-screen py-20 px-6 md:px-16 relative"
    >
      {/* Background & Overlay */}
      <div className="absolute inset-0 w-full h-full bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/assets/images/section2.jpg')", opacity: 0.65 }}></div>
      <div className="absolute inset-0 bg-gradient-to-b from-gray-900/80 via-transparent to-gray-900/80"></div>

      {/* Section Title */}
      <div className={`text-center mb-12 transition-all duration-1000 relative z-10 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Fitur <span className="text-blue-400">Unggulan</span></h2>
        <p className="text-gray-300 max-w-2xl mx-auto">Setiap varian Gondo dirancang dengan fitur-fitur terbaik untuk memberikan pengalaman berkendara yang tak tertandingi.</p>
      </div>

      {/* Varian Tabs */}
      <div className={`flex flex-wrap justify-center gap-4 mb-12 transition-all duration-1000 delay-200 relative z-10 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      ...
      {/* Detail Varian */}
      <div className={`grid grid-cols-1 lg:grid-cols-2 gap-10 items-center mb-16 transition-all duration-1000 delay-300 relative z-10 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="backdrop-blur-sm bg-gradient-to-b from-gray-800/60 to-gray-900/80 border border-gray-700/40 rounded-xl p-6 flex justify-center items-center">
          <img src={cars[activeTab].image} alt={`Gondo ${activeTab}`} className="max-h-80 object-contain transform hover:scale-105 transition-transform duration-500" />
        </div>
        <div className="backdrop-blur-sm bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border border-blue-500/30 rounded-xl p-8">
          <h3 className="text-2xl font-bold text-white mb-2">Gondo {activeTab}</h3>
          <p className="text-gray-300">{cars[activeTab].tagline}</p>
          <p className="text-blue-400 font-semibold mt-2">{cars[activeTab].price}</p>
          <h4 className="text-lg font-semibold text-white mt-6 mb-4">Fitur Unggulan:</h4>
      ...
      {/* Fitur Kenyamanan */}
      <h3 className={`text-2xl font-bold text-white mb-8 text-center relative z-10 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
